...
export default function LoginPage({ onSwitch }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    setLoading(true);
    try {
      const mockGoogleEmail = prompt('Enter your Google email:');
      if (mockGoogleEmail) {
        await googleLogin(mockGoogleEmail, mockGoogleEmail.split('@')[0]);
      }
    } catch (err) {
      setError(err.message);
    } finally {
      ...
      <div className="auth-container animate-fade-up">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <button type="submit" className="auth-btn primary-btn" disabled={loading}>
            {loading ? 'Authenticating...' : mfaRequired ? 'Verify OTP' : 'Sign In'}
          </button>
        </form>
